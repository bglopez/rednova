...
while (($res>0)&&($i<99))
	{
		$i++;
		$planetname = substr($playerinfo[character_name],0,1) . substr($playerinfo[ship_name],0,1) . "-" . $playerinfo[sector] . "-" . $i;
		$res = $db->Execute("SELECT planet_id FROM $dbtables[planets] WHERE name = '$planetname'");
		$res = $res->fields[planet_id];
		// echo "SELECT planet_id FROM $dbtables[planets] WHERE name = '$planetname' ==> $res<BR>";
	}
// Generate Planetname END
  ...
  elseif($zoneinfo[allow_planet] == 'L')
  {
    if($zoneinfo[corp_zone] == 'N')
    {
      ...
      else
      {
        ...
        {
          $query1 = "INSERT INTO $dbtables[planets] VALUES('', $playerinfo[sector], '$planetname', 0, 0, 0, 0, 0, 0, 0, 0, $playerinfo[ship_id], 0, 'N', 'N', $default_prod_organics, $default_prod_ore, $default_prod_goods, $default_prod_energy, $default_prod_fighters, $default_prod_torp, 'N')";
          ...
          $res = $db->Execute("SELECT planet_id FROM $dbtables[planets] WHERE name = '$planetname'");
          AddLog("PGC",$playerinfo[sector],$res->fields[planet_id]);
          echo $l_gns_pcreate;
        }
      }
    }
