...
if($playerinfo[ship_fighters] < 1)
    {
        echo "You need fighters to perform SOFA ! The BUG is crushed.<br><br>\n";
        TEXT_GOTOMAIN();
        include("footer.php");
        die();
    }
...
$Logging = "PFC: $ownerfightercapacity  AFC: $attackerfightercapacity";
...
$Logging .= "PB: $planetbeams  BU: $beamsused";

    if(($attackerfighters<=$attackerfighterslost)&&($beamsused>0))
    {
        echo "$l_bigbeams<br>\n";
        if ($debug) echo "Fighters destroyed by beams $attackerfighterslost<BR>\n";
$Logging .= " => Beam killed all";
    }
    else
    {
...
// INIZIO MODIFICA TORPS BUG
        $torpsused=$planettorps;
        if ($torpsused*$torp_dmg_rate >$attackerfighters-$attackerfighterslost)
             {
             $torpsused = round(($attackerfighters-$attackerfighterslost)/$torp_dmg_rate)+1;
             }

        if ($torpsused>$planettorps) $torpsused=$planettorps;
        $attackerfighterslost+=$torpsused*$torp_dmg_rate;
// FINE MODIFICA TORPS BUG
$Logging .= "<BR>PT: $planettorps";
        if(($attackerfighters<=$attackerfighterslost)&&($planettorps>0))
                        {
            echo "$l_bigtorps<br>\n";
$Logging .= " => Torp killed all";
                        }
        else
        {
...
$Logging .= "<BR>PFL: $planetfighterslost($planetfighters)  AFL: $attackerfighterslost($attackerfighters)";
AddLog("PS",$planetinfo[planet_id],$Logging);
}
...
function planetcombat()
{
    ...
    // Line replaced because of energy bug!
...
$Logging = "AF: $attackerfighters  AT: $attackertorps  AB: $attackerbeams<BR>";
$Logging .= "PF: $planetfighters  PT: $planettorps  PB: $planetbeams<BR>";
...
$Logging .= "A: Torps breached armor<BR>";
        }
        else
        {
...
$Logging .= " with ESCAPE<BR>";
    ...
    if($planetshields < 1 && $planetfighters < 1 && $attackerarmor > 0 && $shipsonplanet == 0)
    {
        ...
        if($min_value_capture != 0)
        {
            ...
            {
AddLog("PAK",$planetinfo[planet_id],$playerinfo[sector]);
            ...
            {
AddLog("PAD",$planetinfo[planet_id],$playerinfo[sector]);
        ...
        {
AddLog("PAD2",$planetinfo[planet_id],$playerinfo[sector]);
...
$Logging = "Sector: $targetinfo[sector]<BR>";
$Logging .= "AF:$attackerfighters AB:$attackerbeams AT:$attackertorps AS:$attackershields AA:$attackerarmor<BR>";
...
$Logging .= "T: Torps breached Armor<BR>";
        }
        else
        {
...
$Logging .= "A: Armor breached by Torps<BR>";
        }
        else
        {
...
$Logging .= "T: Armor breached with Fighter<BR>";
        }
        else
        {
...
$Logging .= "T: Armor HIT with fighter<BR>";
        }
    }
    else
    {
...
$Logging .= "A: Armor breached with fighter<BR>";
        }
        else
        {
...
$Logging .= "A: Armor HIT with fighter<BR>";
        }
    }
    else
    {
...
$Logging .= "T: destroyed but excaped<BR>";
        }
        else
        {
...
AddLog("SSC",$targetinfo[ship_id],$Logging);
}
?>