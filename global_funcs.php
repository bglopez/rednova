...
if(!empty($lang))
{
  ...
  foreach($avail_lang as $key => $value)
  {
    if($lang == $value[file])
    {
      SetCookie("lang",$lang,time()+$Timeout,$gamepath,$gamedomain);
      $found = 1;
      break;
    }
  }
...
$dbtables['vote']       = "${db_prefix}rnt_vote";
$dbtables['shoutbox']   = "${db_prefix}rnt_shoutbox";
$dbtables['adminnews']  = "${db_prefix}rnt_adminnews";
$dbtables['hof_game']   = "${db_prefix}rnt_hof_game";
$dbtables['hof_player'] = "${db_prefix}rnt_hof_player";
$dbtables['adm_logs']   = "${db_prefix}rnt_logging";
$dbtables['rpp_pay']    = "${db_prefix}rnt_pay";
...
function checklogin()
{
  ...
  global $db, $dbtables, $ip, $Timeout, $rednova;
...
if($playerinfo['cheater'] == "Y") {
		global $admin_mail;
		echo "Your account is locked!<BR><BR>";
		echo "The reason can be:<BR>";
		echo "- You are a mult-account cheater<BR>";
		echo "- You are playing from one PC with other people<BR>";
		echo "- Scanned the Universe<BR>";
		echo "<BR>If you think this is a mistake, please contact the <A HREF='mailto:$admin_mail?Subject=Account Locked'>Administrator</A>. (Add your player name in the mail!)";
		$flag = 1;
	}

	if(($playerinfo['turns_used']>500)&&($playerinfo['score']<800)&&($flag!=1)&&($playerinfo[pay]!="Y")&&($playerinfo['cheater']!="O")) {
			$current_score = gen_score($playerinfo[ship_id]);
			if($current_score<1000) {
					$sql = "SELECT count(event_id) as cc FROM $dbtables[movement_log] WHERE ship_id = $playerinfo[ship_id]";
					$result2 = $db->Execute($sql);
					$row2 = $result2->fields;
					if( $row2[cc] > ($playerinfo['turns_used']/15) ) {
							global $admin_mail;
							echo "Your account is locked!<BR><BR>";
							echo "The reason can be:<BR>";
							echo "- Scanned the Universe<BR>";
							...
							$sql = "UPDATE $dbtables[ships] SET cheater = 'Y' WHERE ship_id = $playerinfo[ship_id]";
							$db->Execute($sql);
							$sql = "INSERT INTO bnt_blacklist (email,reason) VALUES ('$playerinfo[email]','SCN')";
							$db->Execute($sql);
						}
				}
		}
                ...
                if((!isSet($rednova))||($rednova==null))
                        {
                                ...
                                AddLog("SLK",$tmp,"",$playerinfo);
                        } else {
                                $sql = "SELECT LogPlayer FROM $dbtables[adm_logs] WHERE LogPlayer != $playerinfo[ship_id] AND LogData1 = '$rednova' AND LogEvent LIKE 'SL_' ORDER BY LogTime DESC LIMIT 1";
                                $res2 = $db->Execute($sql);
                                $tmp = $res2->fields[LogPlayer];
                                //$rednovaID = substr($rednova, 0, strpos($rednova, "#"));
                                if (($tmp!=$playerinfo[ship_id])&&($tmp!=null))
                                        {
                                        $sql = "SELECT count(*) as c FROM $dbtables[adm_logs] WHERE LogEvent = 'SPK' AND LogPlayer = " . $playerinfo[ship_id];
                                        $res2 = $db->Execute($sql);
                                        $tmp2 = $res2->fields[c];
                                        if($tmp2==0)
                                                AddLog("SPK",$rednova,$tmp,$playerinfo);
                                        } else {
                                        $sql = "SELECT LogPlayer FROM $dbtables[adm_logs] WHERE LogIP = '$ip' AND LogPlayer != $playerinfo[ship_id] AND LogTime >= " . (time() - (3600*24) ) . " ORDER BY LogTime DESC LIMIT 1";
                                        $res2 = $db->Execute($sql);
                                        $tmp = $res2->fields[LogPlayer];
                                        if (($tmp!=$playerinfo[ship_id])&&($tmp!=null))
                                                {
                                                $sql = "SELECT count(*) as c FROM $dbtables[adm_logs] WHERE LogEvent = 'SPI' AND LogIP = '$ip' AND LogPlayer = " . $playerinfo[ship_id];
                                                $res2 = $db->Execute($sql);
                                                $tmp2 = $res2->fields[c];
                                                if($tmp2==0)
                                                        AddLog("SPI",$rednova,$tmp,$playerinfo);
                                                }
...
function updatecookie()
{
  ...
  SetCookie( "userpass" , $userpass,time() + $Timeout , $gamepath , $gamedomain );
  if ($userpass != '' and $userpass != '+') {
...
$res = $db->Execute("SELECT $calc_levels+$calc_equip+$calc_dev+$dbtables[ships].credits AS score FROM $dbtables[ships]  WHERE ship_id=$sid AND ship_destroyed='N'");
$row = $res->fields;
$score1 = $row[score];
$res = $db->Execute("SELECT $calc_planet_goods+$calc_planet_colonists+$calc_planet_defence+$calc_planet_credits AS score FROM $dbtables[planets] WHERE owner=$sid");
$row = $res->fields;
$score2 = $row[score];
$score=$score1+$score2;
...
function destroy_fighters($sector, $num_fighters)
{
    ...
    if($result3 > 0)
    {
       while(!$result3->EOF && $num_fighters > 0)
       {
          ...
          {
//             $update = $db->Execute("DELETE FROM $dbtables[sector_defence] WHERE defence_id = $row[defence_id]");
             $update = $db->Execute("UPDATE $dbtables[sector_defence] set quantity=0 WHERE defence_id = $row[defence_id]");
...
function admLogging($LogPlayer,$LogIP,$LogTime,$LogEvent,$LogData1,$LogData2)
        {
                global $dbtables,$db;
                $part1 = "INSERT INTO " . $dbtables['adm_logs'] . " (LogPlayer,LogIP,LogTime,LogEvent";
                        if ((isSet($LogData1))&&($LogData1!=null)) $part1 .= ",LogData1";
                        if ((isSet($LogData2))&&($LogData2!=null)) $part1 .= ",LogData2";
                $part2 = ") VALUES ($LogPlayer,'$LogIP',$LogTime,'$LogEvent'";
                        if ((isSet($LogData1))&&($LogData1!=null)) $part2 .= ",'$LogData1'";
                        if ((isSet($LogData2))&&($LogData2!=null)) $part2 .= ",'$LogData2'";
                $sql = $part1 . $part2 . ")";
                $db->Execute($sql);
        }

function AddLog($LogEvent,$LogData1 = "",$LogData2 = "",$PL = "")
        {
                global $dbtables,$db,$ip,$playerinfo;
                global $advlog_cookiecount,$advlog_ban_cookiecount,$advlog_ban_wrongkey,$advlog_ban_wrongip;
                if ($PL != "") $playerinfo = $PL;

                if($LogEvent=="SLO")
                        {
                        $sql = "SELECT cookie FROM $dbtables[ships] WHERE ship_id = $playerinfo[ship_id]";
                        $res3 = $db->Execute($sql);
                        if($res3->fields[cookie]==0)
                                {
                                ...
                                if(rand(0,30)==0)
                                        ...
                                        $tmp = $playerinfo[ship_id] . "#" . time();
                                        SetCookie("rednova",$tmp,time()+(150),$gamepath,$gamedomain);
                                        $sql = "UPDATE $dbtables[ships] SET cookie = cookie - 1 WHERE ship_id = $playerinfo[ship_id]";
                                        $db->Execute($sql);
                                        }
                                }
                        }
                if($LogEvent=="SLN")
                        ...
                        if($res3->fields[cookie]<0)
                                ...
                                $sql = "SELECT cookie FROM $dbtables[ships] WHERE ship_id = $playerinfo[ship_id]";
                                $res3 = $db->Execute($sql);
                                if(($res3->fields[cookie]>$advlog_cookiecount)&&($advlog_ban_cookiecount))
                                        {
                                        $sql = "UPDATE $dbtables[ships] SET cheater = 'Y' WHERE ship_id = $playerinfo[ship_id] AND pay = 'N' AND cheater != 'O'";
                                        $db->Execute($sql);
													 //$sql = "INSERT INTO bnt_blacklist (email,reason) VALUES ('$playerinfo[email]','SLN')";
													 //$db->Execute($sql);
                                        }
                                }
                        }
                if($LogEvent=="SLK")
                        ...
                        if(($res3->fields[cookie]>$advlog_cookiecount)&&($advlog_ban_cookiecount))
                                ...
                                $sql = "UPDATE $dbtables[ships] SET cheater = 'Y' WHERE ship_id = $playerinfo[ship_id] AND pay = 'N' AND cheater != 'O'";
                                $db->Execute($sql);
											//$sql = "INSERT INTO bnt_blacklist (email,reason) VALUES ('$playerinfo[email]','SLK')";
											//$db->Execute($sql);
                                }

                        }
                if(($LogEvent=="SPK")&&($advlog_ban_wrongkey))
                        {
                        $sql = "UPDATE $dbtables[ships] SET cheater = 'Y' WHERE ship_id = $playerinfo[ship_id] AND pay = 'N' AND cheater != 'O'";
                        $db->Execute($sql);
                        $sql = "UPDATE $dbtables[ships] SET cheater = 'Y' WHERE ship_id = $LogData2 AND pay = 'N' AND cheater != 'O'";
                        $db->Execute($sql);
								//$sql = "INSERT INTO bnt_blacklist (email,reason) VALUES ('$playerinfo[email]','SPK')";
								//$db->Execute($sql);
                        }
                if(($LogEvent=="SPI")&&($advlog_ban_wrongip))
                        {
                        $sql = "UPDATE $dbtables[ships] SET cheater = 'Y' WHERE ship_id = $playerinfo[ship_id] AND pay = 'N' AND cheater != 'O'";
                        $db->Execute($sql);
                        $sql = "UPDATE $dbtables[ships] SET cheater = 'Y' WHERE ship_id = $LogData2 AND pay = 'N' AND cheater != 'O'";
                        $db->Execute($sql);
								//$sql = "INSERT INTO bnt_blacklist (email,reason) VALUES ('$playerinfo[email]','SPI')";
								//$db->Execute($sql);
                        }


                admLogging($playerinfo[ship_id],$ip,time(),$LogEvent,$LogData1,$LogData2);
