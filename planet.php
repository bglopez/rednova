...
/* if there is a planet in the sector show appropriate menu */
{
  if($playerinfo[sector] != $planetinfo[sector_id])
  {
    ...
    TEXT_GOTOMAIN();
    AddLog("PPN",$playerinfo[sector],$planetinfo[planet_id]);
    include("footer.php");
    die();
  }
  ...
  if(empty($command))
  {
    ...
    { 
       ...
       { 
          ...
          { 
             AddLog("PGD",$playerinfo[sector],$planet_id);
...
// Generate Name * end
AddLog("PC",$planet_id,$planetinfo[credits]);
      echo "$l_planet_captured<BR>";
      $update = $db->Execute("UPDATE $dbtables[planets] SET corp=null, owner=$playerinfo[ship_id], base='N', defeated='N',name='$planetname' WHERE planet_id=$planet_id");
      $ownership = calc_ownership($playerinfo[sector]);
