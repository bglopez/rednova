...
function traderoute_create()
{
  ...
  if($ptype1 == 'port')
  {
    ...
    if(!$query || $db->EOF)
    {
      ...
      traderoute_die($l_tdr_errnotvalidport);
		...
		//$p1query = $db->Execute("SELECT * FROM $dbtables[movement_log] WHERE sector_id=$port_id1 AND ship_id = $playerinfo[ship_id]");
		//$num_res1 = $p1query->numRows();
		//if($num_res1 == 0)
		//{
		//  traderoute_die("You must first visit sector $port_id1 prior to creating a trouderoute there!");
		//}
  ...
  else
  {
    ...
    $source=$query->fields;
      ...
      traderoute_die($l_tdr_errnosrc);
		...
		if($num_res1 == 0)
		{
		  traderoute_die("You must first visit the sector that the planet resides in prior to creating a trouderoute using it!");
		}
  ...
  if($ptype2 == 'port')
  {
    ...
    $destination=$query->fields;
		...
		//$p2query = $db->Execute("SELECT * FROM $dbtables[movement_log] WHERE sector_id=$port_id2 AND ship_id = $playerinfo[ship_id]");
		//$num_res2 = $p2query->numRows();
		//if($num_res2 == 0)
		//{
		//  traderoute_die("You must first visit sector $port_id2 prior to creating a trouderoute there!");
		//}
...
//--------------------------------------------------------------------------------
  echo "<table border=1 cellspacing=1 cellpadding=2 width=\"65%\" align=center>".
    "<tr bgcolor=$color_line2><td align=\"center\" colspan=7><b><font color=white>$l_tdr_tdrres</font></b></td></tr>".
    "<tr align=center bgcolor=$color_line2>".
...
// ------------ Determine if Destination is Planet or Port
  ...
  echo "</b></font></td></tr><tr bgcolor=$color_line1><td align=center><font size=2 color=white>";
