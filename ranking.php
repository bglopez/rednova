...
$res = $db->Execute("SELECT COUNT(*) AS num_players FROM $dbtables[ships] WHERE ship_destroyed='N' and character_name != 'WebMaster'");
$row = $res->fields;
...
$res = $db->Execute("SELECT $dbtables[ships].email,$dbtables[ships].cheater,$dbtables[ships].score,$dbtables[ships].character_name,$dbtables[ships].turns_used,$dbtables[ships].last_login,UNIX_TIMESTAMP($dbtables[ships].last_login) as online,$dbtables[ships].rating, $dbtables[teams].team_name, IF($dbtables[ships].turns_used<150,0,ROUND($dbtables[ships].score/$dbtables[ships].turns_used)) AS efficiency FROM $dbtables[ships] LEFT JOIN $dbtables[teams] ON $dbtables[ships].team = $dbtables[teams].id  WHERE ship_destroyed='N' and character_name != 'WebMaster' ORDER BY $by LIMIT $max_rank");
...
else
{
  ...
  while(!$res->EOF)
  {
    ...
    if(preg_match("/@furangee/i", $row[email])) { $online = " "; $row[team_name] = "<IMG SRC='./images/star.gif'>Furangee"; }
    if($row[cheater]=="Y") { $online = "&nbsp;&nbsp;&nbsp;";  }

    echo "<TR BGCOLOR=\"$color\"><TD NOWRAP>" . NUMBER($i) . "</TD><TD NOWRAP ALIGN=RIGHT>" . NUMBER($row[score]) . "</TD><TD NOWRAP>";
    //echo "&nbsp;";
    //echo player_insignia_name($row[email]);
    echo "&nbsp;";
    echo "<b>$row[character_name]</b></TD><TD NOWRAP ALIGN=RIGHT>" . NUMBER($row[turns_used]) . "&nbsp;</TD><TD NOWRAP ALIGN=CENTER>$row[last_login]</TD><TD NOWRAP ALIGN=RIGHT>&nbsp;&nbsp;" . NUMBER($rating) . "&nbsp;</TD><TD NOWRAP>$row[team_name]&nbsp;</TD><TD NOWRAP>$online</TD><TD NOWRAP ALIGN=RIGHT>$row[efficiency]</TD></TR>\n";
